...
func TestModuleInstance_Memory(t *testing.T) {
	...
	for _, tt := range tests {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			instance, err := s.Instantiate(testCtx, tc.input, "test", nil, nil, -1)
...
func TestStore_Instantiate(t *testing.T) {
	...
	mod, err := s.Instantiate(testCtx, m, "bar", sysCtx, []FunctionTypeID{0}, -1)
	require.NoError(t, err)
...
func TestStore_CloseWithExitCode(t *testing.T) {
	...
	for _, tt := range tests {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			}, importedModuleName, nil, []FunctionTypeID{0}, -1)
			require.NoError(t, err)
			...
			}, importingModuleName, nil, []FunctionTypeID{0}, -1)
			require.NoError(t, err)
...
func TestStore_hammer(t *testing.T) {
	...
	s := newStore()
	imported, err := s.Instantiate(testCtx, m, importedModuleName, nil, []FunctionTypeID{0}, -1)
	require.NoError(t, err)
	...
	hammer.NewHammer(t, P, N).Run(func(name string) {
		mod, instantiateErr := s.Instantiate(testCtx, importingModule, name, sys.DefaultContext(nil), []FunctionTypeID{0}, -1)
...
func TestStore_hammer_close(t *testing.T) {
	...
	for i := 0; i < instCount; i++ {
		mod, instantiateErr := s.Instantiate(testCtx, importingModule, strconv.Itoa(i), sys.DefaultContext(nil), []FunctionTypeID{0}, -1)
		require.NoError(t, instantiateErr)
		instances[i] = mod
	}
...
func TestStore_Instantiate_Errors(t *testing.T) {
	...
	t.Run("Fails if module name already in use", func(t *testing.T) {
		s := newStore()
		_, err = s.Instantiate(testCtx, m, importedModuleName, nil, []FunctionTypeID{0}, -1)
		require.NoError(t, err)
	...
	t.Run("creating engine failed", func(t *testing.T) {
		...
		_, err = s.Instantiate(testCtx, importingModule, importingModuleName, nil, []FunctionTypeID{0}, -1)
